...
{  //割り込み内からコール
  ...
  if (con_wall.enable == true) {
    ...
    speed_target_r = speed + omega*tread_width/2.0f;
    speed_target_l = speed - omega*tread_width/2.0f;;
  }
...
void RUN::sura(t_sura_mode dir,int init_speed)
{

  switch(dir){
    case R90:
    case L90:
      if(init_speed ==g_run.search_speed_low){
        oneStep(pre_st_low+sura_rl90_before_len_low,init_speed);
      }else if(init_speed==g_run.search_speed){
        oneStep(pre_st_nor+sura_rl90_before_len,init_speed);
      }else{
        oneStep(pre_st_high+sura_rl90_before_len_high,init_speed);
      }
    break;
    case R90H:
    case L90H:
      if(init_speed==(g_run.search_speed+75)){
        oneStep(pre_st_nor+sura_rl90h_before_len,init_speed);
      }else{
        oneStep(pre_st_high+sura_rl90h_before_len_high,init_speed);
      }
    break;
    case R180:
    case L180:
...
//旋回
  ...
  switch(dir){
    ...
    case L90:
      if(init_speed ==g_run.search_speed_low){
        ...
        obj_step = (int)((45.0f-(float)pre_st_low) * PI / pulse)+sura_rl90_omega_low;//<=旋回角度はここで調整する
      }else if(init_speed==g_run.search_speed){
        ...
        obj_step = (int)((45.0f-(float)pre_st_nor) * PI / pulse)+sura_rl90_omega;//<=旋回角度はここで調整する
      }else{
        ...
        obj_step = (int)((45.0f-(float)pre_st_high) * PI / pulse)+sura_rl90_omega_high;//<=旋回角度はここで調整する        
      }
    break;
    case R90H:
    case L90H:    
      if(init_speed==(g_run.search_speed+75)){
        omega = init_speed/(60.0f-(float)pre_st_nor);
        upper_speed_limit = omega*(60.0f-(float)pre_st_nor + tread_width/2.0f);
        lower_speed_limit = omega*(60.0f-(float)pre_st_nor - tread_width/2.0f);
        obj_step = (int)((60.0f-(float)pre_st_nor) * PI / pulse)+sura_rl90h_omega;//<=旋回角度はここで調整する        
      }else{
        omega = init_speed/(60.0f-(float)pre_st_high);
        upper_speed_limit = omega*(60.0f-(float)pre_st_high + tread_width/2.0f);
        lower_speed_limit = omega*(60.0f-(float)pre_st_high - tread_width/2.0f);
        obj_step = (int)((60.0f-(float)pre_st_high) * PI / pulse)+sura_rl90h_omega_high;//<=旋回角度はここで調整する
      }
    break;
    case R180:
    case L180:
      if(init_speed==(g_run.search_speed+50)){
        omega = init_speed/(45.0f)-0.6;//<=センターずれはここで調整する
        upper_speed_limit = omega*(45.0f+ tread_width/2.0f);
        lower_speed_limit = omega*(45.0f- tread_width/2.0f);
        obj_step = (int)(90.0f * PI / pulse)+sura_rl180_omega;//<=旋回角度はここで調整する        
      }else{
        omega = init_speed/(45.0f);//<=センターずれはここで調整する
        upper_speed_limit = omega*(45.0f+ tread_width/2.0f);
        lower_speed_limit = omega*(45.0f- tread_width/2.0f);
  ...
  switch(dir){
    ...
    case L90:
      if(init_speed ==g_run.search_speed_low){
        oneStep(pre_st_low+sura_rl90_after_len_low,init_speed);
      }else if(init_speed==g_run.search_speed){
        oneStep(pre_st_nor+sura_rl90_after_len,init_speed);
      }else{
        oneStep(pre_st_high+sura_rl90_after_len_high,init_speed);
      }
    break;
    case R90H:
    case L90H:
      if(init_speed==(g_run.search_speed+75)){
        oneStep(pre_st_nor+sura_rl90h_after_len,init_speed);
      }else{
        oneStep(pre_st_high+sura_rl90h_after_len_high,init_speed);
      }
    break;
    case R180:
    case L180:
...
void RUN::back(int len)
{
  int obj_step;

  controlInterruptStop();
  accel = 1.0;
  speed = lower_speed_limit = upper_speed_limit = (double)MIN_SPEED;
  upper_speed_limit = 200;
  con_wall.enable = false;
