...
void RUN::sura(t_local_direction dir,int init_speed)
{
  ...
  }else if(init_speed==(search_speed_high-25)){
    oneStep(pre_st_high-5,init_speed);
  }else if(init_speed==(search_speed+50)){
    oneStep(pre_st_nor-5,init_speed);
  }else{
    oneStep(pre_st_high-5,init_speed);
  }
  ...
  if(init_speed==(search_speed_low-25)){
    ...
    obj_step = (int)((60.0f-(float)pre_st_nor) * PI / pulse)-0;//<=旋回角度はここで調整する
  }else {
    ...
    obj_step = (int)((60.0f-(float)pre_st_high) * PI / pulse)-0;//<=旋回角度はここで調整する
  }

  speed = init_speed;
  if(dir==right)omega=omega* -1.0f;
  con_wall.enable = false;
  dirSet(MOT_FORWARD, MOT_FORWARD);
  speedSet(speed, speed);
  counterClear();
