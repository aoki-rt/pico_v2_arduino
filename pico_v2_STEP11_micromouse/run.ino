...
#define pre_st 10.0
void RUN::sura(t_local_direction dir)
{

  oneStep(pre_st,search_speed);

  int obj_step;
  controlInterruptStop();
  accel = 0.0;//円弧で旋回するので加減速はしなし
  omega = search_speed/(45.0f-(float)pre_st);
  upper_speed_limit = omega*(45.0f-(float)pre_st + tread_width/2.0f);
  speed = search_speed;
  lower_speed_limit = omega*(45.0f-(float)pre_st - tread_width/2.0f);
  if(dir==right)omega=omega* -1.0f;
  con_wall.enable = false;
  obj_step = (int)((45.0f-(float)pre_st) * PI / pulse);
  dirSet(MOT_FORWARD, MOT_FORWARD);
  speedSet(speed, speed);
    counterClear();
  controlInterruptStart();
