...
const char* ssid_misc = "使用しているルータのSSID";
const char* password_misc = "ルータのパスワード";
...
void MISC::modeExec(int mode) {
  ...
  switch (mode) {
    ...
    case 4:  //足立法 sura-low
      g_run.positionInit();
      max_speed_tmp = g_run.max_speed;
      g_run.max_speed = (g_run.search_speed_low + g_run.max_speed) / 2;
      g_map.positionInit();
      g_search.adachi_sura(g_map.goal_mx, g_map.goal_my, g_run.search_accel_low, g_run.search_speed_low);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      g_misc.goalAppeal();
      g_search.adachi_sura(0, 0, g_run.search_accel_low, g_run.search_speed_low);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      mapWrite();
      g_run.max_speed = max_speed_tmp;
      break;
    case 5:  //足立法 sura-normal
      g_run.positionInit();
      max_speed_tmp = g_run.max_speed;
      g_run.max_speed = (g_run.search_speed + g_run.max_speed) / 2;
      g_map.positionInit();
      g_search.adachi_sura(g_map.goal_mx, g_map.goal_my, g_run.search_accel, g_run.search_speed);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      g_misc.goalAppeal();
      g_search.adachi_sura(0, 0, g_run.search_accel, g_run.search_speed);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      mapWrite();
    ...
    case 6:
      ...
      mapCopy();
      g_map.positionInit();
      g_fast.runPatternSura(g_map.goal_mx, g_map.goal_my, g_run.search_accel, g_run.search_speed);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      g_misc.goalAppeal();
    ...
    case 7:
      ...
      mapCopy();
      g_map.positionInit();
      g_fast.runPatternSura(g_map.goal_mx, g_map.goal_my, g_run.search_accel_high, g_run.search_speed_high);
      g_run.rotate(right, 2);
      g_map.nextDir(right);
      g_map.nextDir(right);
      g_misc.goalAppeal();
    ...
    case 14:
      motorDisable();
      WiFi.mode(WIFI_STA);
      WiFi.begin(ssid_misc, password_misc);
