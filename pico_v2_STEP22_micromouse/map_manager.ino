...
unsigned short MapManager::weightDataGet(unsigned char x, unsigned char y) {
  ...
  do {
    ...
    for (int i = 0; i < MAZESIZE_X; i++) {
      for (int j = 0; j < MAZESIZE_Y; j++) {
        if (steps_map[i][j] == 65535) continue;
        if (
          (j < (MAZESIZE_Y - 1)) && (wall[i][j].north == NOWALL) && (steps_map[i][j + 1] == 65535)) {
          steps_map[i][j + 1] = steps_map[i][j] + 1;
          change_flag = true;
        }
...
void MapManager::map2Make2(int x, int y) {
  bool change_flag;
  ...
  do {
    ...
    for (int i = 0; i < MAZESIZE_X; i++) {
      for (int j = 0; j < MAZESIZE_Y; j++) {
        if (steps_map[i][j] == 65535) continue;
        //北側の調査
        if ((j < (MAZESIZE_Y - 1)) && (wall[i][j].north == NOWALL)) {
          if (tmp_dir[i][j] == north) {
            if (steps_map[i][j + 1] == 65535) {  //未探索区画
              steps_map[i][j + 1] = steps_map[i][j] + 1;
              tmp_dir[i][j + 1] = north;
              change_flag = true;
            } else {
              if (steps_map[i][j + 1] > (steps_map[i][j] + 1)) {
                steps_map[i][j + 1] = steps_map[i][j] + 1;
                tmp_dir[i][j + 1] = north;
                change_flag = true;
              }
            ...
            if ((steps_map[i][j + 1] == 65535)) {  //未探索区画
              steps_map[i][j + 1] = steps_map[i][j] + 3;
              tmp_dir[i][j + 1] = north;
              change_flag = true;
            } else {
              if (steps_map[i][j + 1] > (steps_map[i][j] + 3)) {
                steps_map[i][j + 1] = steps_map[i][j] + 3;
                tmp_dir[i][j + 1] = north;
                change_flag = true;
              }
        ...
        //東側の調査
        if ((i < (MAZESIZE_X - 1)) && (wall[i][j].east == NOWALL)) {
          if (tmp_dir[i][j] == east) {
            if (steps_map[i + 1][j] == 65535) {
              ...
              if (steps_map[i + 1][j] > (steps_map[i][j] + 1)) {
                steps_map[i + 1][j] = steps_map[i][j] + 1;
                tmp_dir[i + 1][j] = east;
                change_flag = true;
              }
            ...
            if (steps_map[i + 1][j] == 65535) {
              steps_map[i + 1][j] = steps_map[i][j] + 3;
              tmp_dir[i + 1][j] = east;
              change_flag = true;
            } else {
              if (steps_map[i + 1][j] > (steps_map[i][j] + 3)) {
                steps_map[i + 1][j] = steps_map[i][j] + 3;
                tmp_dir[i + 1][j] = east;
                change_flag = true;
              }
        ...
        //南側の調査
        if ((j > 0) && (wall[i][j].south == NOWALL)) {
          if (tmp_dir[i][j] == south) {
